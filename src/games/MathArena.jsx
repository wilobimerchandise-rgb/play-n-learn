...
export default function MathArena({ onBack, addCoins, addXp, showToast }) {
  ...
  useEffect(() => {
    if (timer <= 0) {
      ...
      showToast("Too slow! Monster hit you!");
      newQuestion();
      return;
    }
    ...
    if(numAnswer === question.answer) {
      setMonsterHp(h => {
        ...
        if(newHp <= 0) {
          ...
          setPlayerHp(100);
          setTimeout(() => setMonsterHp(100), 500);
        }
        return newHp;
      });
      ...
      showToast("Wrong! Try again champ! 💪");
    }
    setAnswer(''); 
    newQuestion();
  }
  ...
  return (
    <div style={{minHeight: '100vh', background: 'linear-gradient(135deg, #581c87, #a21caf)', padding: 16, color: 'white', fontFamily: 'sans-serif'}}>
      <button onClick={onBack} style={{marginBottom: 16, background: 'white', color: '#581c87', padding: '10px 20px', borderRadius: 12, fontWeight: 'bold', border: 'none', fontSize: 16, cursor: 'pointer'}}>
        ← Back Home
      </button>
      ...
      <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: 32, maxWidth: 500, margin: '0 auto 32px'}}>
        <div>
          <div style={{fontWeight: 'bold'}}>You ❤️</div>
      ...
      <div style={{textAlign: 'center'}}>
        ...
        <input 
          ...
          autoFocus
        />
        <button onClick={handleAttack} style={{marginLeft: 16, background: '#facc15', color: 'black', padding: '12px 24px', borderRadius: 12, fontWeight: 'bold', border: 'none', fontSize: 18, cursor: 'pointer'}}>
          ATTACK!
        </button>
      </div>
    </div>
